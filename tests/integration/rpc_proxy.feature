@service_name
Feature: RpcProxy and open_session peer from configuration

    RpcProxy and open_session accept either a static service_name or a
    peer key. peer resolves to kontiki.peers.<peer> and is preferred for
    ...
    # ------------------------------------------------------------
    # open_session peer from kontiki.peers
    # ------------------------------------------------------------
    Scenario: open_session resolves the peer service name from kontiki.peers
        Given a service is running with the following configuration
            """
            kontiki:
              service_name: configured-test-service
        ...
        Given a caller service is running with the following configuration
            ...
            logging:
              ...
              root:
                ...
                level: DEBUG
            """
        When I call the open_session_peer method of rpc-proxy-caller with the following parameters
            """
            {}
            """
        Then rpc-proxy-caller should return the result
